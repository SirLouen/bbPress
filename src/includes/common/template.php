...
function bbp_get_breadcrumb($args = array()) {
    ...
    // Topic Tag (or theme compat topic tag)
        ...
        $pre_current_text = sprintf(
            /* translators: %s: Topic tag name with optional edit link */
    ...
    // Edit topic tag
        ...
        $crumbs[] = sprintf(
            '<a href="%1$s" class="bbp-breadcrumb-topic-tag">%2$s</a>',
            esc_url(get_term_link(bbp_get_topic_tag_id(), bbp_get_topic_tag_tax_id())),
            $tag_text
        );
    ...
    // Search
        ...
        $crumbs[] = sprintf(
            '<a href="%1$s" class="bbp-breadcrumb-search">%2$s</a>',
